...
public:
  basedb (const std::string & name, const std::string& dbpath):
    name (name), dbpath(dbpath){ dbinit(); };
  ~basedb (){
  ...
  void dbinit(ldb& dbenv, const std::string& dbpath); // should be called by all derived classes, will use dbpath, name
  void setdb(ldb& dbenv, const std::string& key, const std::string& val, bool append , int& cnt );
  std::string getdb(ldb& dbenv,const std::string& key);
  void dbinit(){
      dbinit(dbenv, dbpath+"/"+name+".ldb");
      }; //default to one db
  void setdb(const std::string& key, const std::string& val, bool append = false){
      setdb(dbenv, key, val, append, cnt);
  };
  std::string getdb(const std::string& key){
      return getdb(dbenv, key);
  };
...
typedef std::unordered_map<std::string, std::vector<std::shared_ptr<basedb>>> DB;
